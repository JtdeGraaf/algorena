  ...
  if (!isAuthenticated) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center">
        <Bot className="h-16 w-16 text-surface-muted" />
        <h2 className="mt-4 text-xl font-semibold">{t('errors.unauthorized')}</h2>
        <p className="mt-2 text-text-secondary">Login to manage your bots</p>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-mono text-2xl font-bold text-primary">$ ./bots --list</h1>
          <p className="mt-1 font-mono text-sm text-text-muted"># Manage your battle bots</p>
        </div>
              ...
              <TerminalTableCell>
                <div className="flex items-center gap-2">
                  <Circle
                    className={cn(
                      'h-2 w-2',
                      bot.active ? 'fill-primary text-primary' : 'fill-surface-muted text-surface-muted'
                    )}
                  />
                  <span className={cn(
                    'text-xs',
                    bot.active ? 'text-primary' : 'text-surface-muted'
                  )}>
                    {bot.active ? 'ACTIVE' : 'IDLE'}
                  </span>
                </div>
              </TerminalTableCell>
              <TerminalTableCell className="font-semibold text-text-primary">
                <div>
                  <div>{bot.name}</div>
                  {bot.description && (
                    <div className="mt-0.5 text-xs text-text-muted">{bot.description}</div>
                  )}
                </div>
              </TerminalTableCell>
              <TerminalTableCell>
